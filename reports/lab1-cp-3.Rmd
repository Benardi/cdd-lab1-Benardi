---
title: "EDA sobre séries no IMDB"
subtitle: 'Existe relação entre tamanho da temporada, número de votos, notas e consenso?'
...
* Nitidamente temos uma moda entre 0 e 250 votos.
* Os maiores valores são muito distantes do valor mais frequente, isso afeta a mediana e a média as tornando maiores que o valor mais frequente. 
* Não existem valores que fujam ao domínio da grandeza (e.g. valores negativos) e que por isso invalidem nossa análise. 

<br></br>

Demos uma olhada nos _10 episódios mais votados_ entre todos:
...
Os top 3 episódios com mais votos são sensivelmente maiores do que o resto, em especial o primeiro colocado que está mais de 40.000 votos acima dos segundo colocado.
...
* A nota mais frequente é nitidamente em torno de 8.
* A distribuição é *relativamente* bem comportada (simétrica), porém existe a ocorrência de notas mais baixas (entre 3 e 6). 
* Não existem valores que fujam ao domínio da grandeza (e.g. valores negativos) e que por isso invalidem nossa análise.     
...
* O tamanho de temporada mais frequente é em torno de 10 e em torno de 20 episódios (Duas modas ou bimodal).
* Algumas poucas temporadas são curiosamente longas. 
* Não existem valores que fujam ao domínio da grandeza (e.g. valores negativos) e que por isso invalidem nossa análise.  
...
* No Q-Q plot  existe uma quebra (descontinuidade), isso casa com o histograma pois temos duas modas (valores mais frequentes).  
